...
public class ExceptionControllerAdvice extends ResponseEntityExceptionHandler {
	...
	public static final String DEFAULT_TITLE = "Input Error Occurred";
	...
	private ProblemDetail createProblemDetail(final HttpClientErrorException exception,
			final HttpStatusCode statusCode) {
		final ProblemDetail problemDetail = ProblemDetail.forStatus(statusCode);
		if (statusCode == HttpStatus.NOT_FOUND) {
			JSONObject jsonObject = new JSONObject(exception.getResponseBodyAsString());
			problemDetail.setDetail(jsonObject.getString("message"));
		} else if (statusCode == HttpStatus.UNAUTHORIZED) {
			problemDetail.setDetail("Invalid API Key");
		}
	...
	private HttpStatusCode getHttpStatusCodeFromSystemException(final SystemException exception) {
		try {
			...
			logger.info(ERROR_MESSAGE + exception.getStatusCode());
